...
「余白」是一款本地优先的随手记应用，用来收好随时闪过的小想法。界面借鉴苹果备忘录的轻量记录体验，同时保留更柔和、走心的视觉气质。
...
当前 Android 安装包仅包含 `arm64-v8a`，以控制体积。浏览器调试可选择「运行 → 运行到浏览器」。根目录的 `index.html` 是 Vue 3 / uni-app H5 调试入口。
